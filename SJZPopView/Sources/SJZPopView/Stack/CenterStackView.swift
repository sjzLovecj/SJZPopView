...
struct CenterStackView: View {
    ...
    @State private var configTemp: PopConfig?
...
// MARK: -Logic Handlers
private extension CenterStackView {
    func onItemsChange(_ items: [AnyPop]) {
...
private extension CenterStackView {
    func showNewHud(_ popup: AnyPop) {
        DispatchQueue.main.async {
            activeView = AnyView(popup.body)
            configTemp = popup.setupConfig(PopConfig())
        }
    }
...
private extension CenterStackView {
    ...
    var config: PopConfig {
        items.last?.setupConfig(PopConfig()) ?? configTemp ?? .init()
    }
}
